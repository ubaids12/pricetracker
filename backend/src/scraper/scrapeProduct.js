...
export async function scrapeProduct(product, options = {}) {
    ...
    const run = (strategy) =>
        strategy === 'browser'
            ? browserScraper(product, options)
            : httpScraper(product, options);

    const attemptLogger = (strategy) => (event) =>
        options.onAttempt?.({
            ...event,
            strategy,
            selectedOption:
